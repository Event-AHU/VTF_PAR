...
def trainer(args,epoch, model,ViT_model, train_loader, valid_loader, criterion, optimizer, scheduler,path,tb_writer,start_epoch):
    ...
    for i in range(start_epoch, epoch+1):
        ...
        tb_writer.add_scalar("lr", optimizer.param_groups[0]['lr'], i )  # 在每次计算完 loss 后添加
        tb_writer.add_scalar("train_loss", train_loss, i )  # 在每次计算完 loss 后添加
        tb_writer.add_scalar("valid_loss", valid_loss, i )  # 在每次计算完 loss 后添加
        tb_writer.add_scalar("valid_mA", average_ma, i )
        ...
        if i % args.epoch_save_ckpt == 0:
            torch.save({
                        ...
                        'group_all':group_all,
                        }, os.path.join(path, f"{time_str()}_epoch{i}_mf1{average_instance_f1*100:.2f}.pth"))             
